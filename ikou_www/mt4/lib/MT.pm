...
#
# $Id: MT.pm.pre 59879 2007-08-01 04:15:34Z bchoate $
...
BEGIN {
    ...
    ( $VERSION, $SCHEMA_VERSION ) = ( '4.0', '4.0026' );
...
sub init_core {
    ...
    $defaults->{HelpURL}{default} = 'http://www.movabletype.jp/documentation/';
...
sub init_callbacks {
    my $mt = shift;
    MT->add_callback( 'build_file_filter', 5, $mt,
...
sub init_plugins {
    ...
    # Always process 'addons' directory
...
sub build_email {
    ...
    eval {
        ...
        $tmpl = MT::Template->new_file( $file, path => \@paths, );
    };
    return $class->trans_error( "Loading template '[_1]' failed: [_2]", $file,
        $@ )
      if $@;
    ...
    my $app          = MT->instance;
    my $old_tmpl_dir = $app->{template_dir};
    MT->instance->{template_dir} = 'email';
    my $body = $tmpl->output;
    $app->{template_dir} = $old_tmpl_dir;
    $class->translate_templatized($body);
}
...
<mtapp:setting
    ...
    hint="<__trans phrase="Sign in using your LiveJournal username.">">
