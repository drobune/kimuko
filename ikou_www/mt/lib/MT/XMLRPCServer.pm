...
sub newPost {
    ...
    for my $f (qw( title description mt_text_more
        ...
        $item->{$f} = MT::I18N::encode_text($item->{$f},'utf-8',$enc);
    ...
    if ($mt->{cfg}->DefaultLanguage eq 'ja') {
        $entry->title($item->{title} || MT::I18N::first_n_text($item->{description}, 10));
    } else {
...
sub getUsersBlogs {
    ...
    while (my $perms = $iter->()) {
        ...
        push @res, { url => SOAP::Data->type(string => $blog->site_url),
                     ...
                     blogName => SOAP::Data->type(string => MT::I18N::encode_text($blog->name,undef,'utf-8')) };
    }
    \@res;
}
...
sub getUserInfo {
    ...
    my($author) = __PACKAGE__->_login($user, $pass);
    die _fault("Invalid login") unless $author;
    my($fname, $lname) = map {MT::I18N::encode_text($_,undef,'utf-8')} split /\s+/, $author->name;
    $lname ||= '';
    { userid => SOAP::Data->type(string => $author->id),
      firstname => SOAP::Data->type(string => $fname),
      lastname => SOAP::Data->type(string => $lname),
      nickname => SOAP::Data->type(string => MT::I18N::encode_text($author->nickname,undef,'utf-8')),
...
sub getRecentPosts {
    ...
    while (my $entry = $iter->()) {
        ...
        if ($class eq 'blogger') {
            $row->{content} = SOAP::Data->type(string => MT::I18N::encode_text($entry->text,undef,'utf-8'));
        } else {
            $row->{title} = SOAP::Data->type(string => MT::I18N::encode_text($entry->title,undef,'utf-8'));
            unless ($titles_only) {
                $row->{description} = SOAP::Data->type(string => MT::I18N::encode_text($entry->text,undef,'utf-8'));
                ...
                $row->{mt_text_more} = SOAP::Data->type(string => MT::I18N::encode_text($entry->text_more,undef,'utf-8'));
                $row->{mt_excerpt} = SOAP::Data->type(string => MT::I18N::encode_text($entry->excerpt,undef,'utf-8'));
                $row->{mt_keywords} = SOAP::Data->type(string => MT::I18N::encode_text($entry->keywords,undef,'utf-8'));
            }
        }
        push @res, $row;
    }
...
sub getPost {
    ...
    my $link = $entry->permalink;
    {
        ...
        userid => SOAP::Data->type(string => $entry->author_id),
        postid => SOAP::Data->type(string => $entry->id),
        description => SOAP::Data->type(string => MT::I18N::encode_text($entry->text,undef,'utf-8')),
        title => SOAP::Data->type(string => MT::I18N::encode_text($entry->title,undef,'utf-8')),
        ...
        mt_text_more => SOAP::Data->type(string => MT::I18N::encode_text($entry->text_more,undef,'utf-8')),
        mt_excerpt => SOAP::Data->type(string => MT::I18N::encode_text($entry->excerpt,undef,'utf-8')),
        mt_keywords => SOAP::Data->type(string => MT::I18N::encode_text($entry->keywords,undef,'utf-8')),
    }
}
...
sub getCategoryList {
    ...
    while (my $cat = $iter->()) {
        push @data, {
            categoryName => SOAP::Data->type(string => MT::I18N::encode_text($cat->label,undef,'utf-8')),
...
sub getTrackbackPings {
    ...
    while (my $ping = $iter->()) {
        push @data, {
            pingTitle => SOAP::Data->type(string => MT::I18N::encode_text($ping->title,undef,'utf-8')),
