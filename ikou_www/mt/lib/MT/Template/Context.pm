...
sub init_default_handlers {
    ...
    $ctx->register_handler(DefaultLanguage => \&_hdlr_default_language);
        ...
        if ($args) {
            ...
            if (my $len = $local_args{trim_to}) {
                if (MT::ConfigMgr->instance->DefaultLanguage eq 'ja') {
                    $str = MT::I18N::substr_text($str, 0, $len);
                } else {
...
sub _hdlr_entry_body {
    ...
    if (MT::ConfigMgr->instance->DefaultLanguage eq 'ja') {
        return MT::I18N::first_n_text($text, $arg->{words}) if exists $arg->{words};
    } else {
...
sub _hdlr_entry_title {
    ...
    if (MT::ConfigMgr->instance->DefaultLanguage eq 'ja') {
        $title = MT::I18N::first_n_text($e->text, 10)
...
sub _hdlr_remote_sign_in_link {
    ...
    return "$signon_url$lang$needs_email&t=$rem_auth_token$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id";
}
...
sub _hdlr_comment_fields {
    ...
    my $rem_auth_token = $blog->effective_remote_auth_token() || "";
    die MT->translate("To enable comment registration, you need to add a TypeKey token "
...
if (getCookie("commenter_name")) {
    ...
    document.write('<MT_TRANS phrase="You are not signed in. You need to be registered to comment on this site."> <a href="$signon_url$lang$needs_email&t=$rem_auth_token&v=$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id"><MT_TRANS phrase="Sign in"></a>');
...
if (getCookie("commenter_name")) {
    ...
    document.write('<MT_TRANS phrase="If you have a TypeKey identity, you can "><a href="$signon_url$lang$needs_email&t=$rem_auth_token&v=$tk_version&_return=$path$comment_script%3f__mode=handle_sign_in%26$static_arg%26entry_id=$entry_id"> <MT_TRANS phrase="sign in"></a> <MT_TRANS phrase="to use it here.">');
}
// -->
</script>
HTML
...
<MT_TRANS phrase="Remember me?">
<input type="radio" id="remember" name="bakecookie" onclick="rememberMe(this.form)" /><label for="remember"><MT_TRANS phrase="Yes"></label><input type="radio" id="forget" name="bakecookie" onclick="forgetMe(this.form)" value="Forget Info" style="margin-left: 15px;" /><label for="forget"><MT_TRANS phrase="No"></label><br style="clear: both;" /></p>
