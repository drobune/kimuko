...
#
# $Id: App.pm,v 1.119.2.1 2004/10/06 22:46:46 ezra Exp $
...
sub login {
    ...
    if ($cookies->{$COOKIE_NAME}) {
        ($user, $pass, $remember) = split /::/, $cookies->{$COOKIE_NAME}->value;
        $crypted = 1;
    }
    ...
    if (my $author = $user_class->load({ name => $user, type => AUTHOR })) {
...
sub run {
    ...
    if (my $url = $app->{redirect}) {
        if ($app->{redirect_use_meta}) {
            ...
            $app->print('<meta http-equiv="refresh" content="0;url=' . $app->{redirect} . '">');
        } else {
            if ($ENV{MOD_PERL}) {
